 ...
 * @Last Modified time: 2018-01-24 12:36:00
 */
/**
 * cgi登录，解析
...
function protocal_login(user, pass)
 {
    return protocal_checkresult("login",user, protocal_callcgi_resystem("login:"+user+"&"+pass))
 }
 ...
 */
function protocal_get_resystem(name,values) {
    var result;
    if (values==null){
        result = protocal_callcgi_resystem("get:" + name );
    }else{
        result = protocal_callcgi_resystem("get:" + name + "&" + values);
    }
    return protocal_checkresult("get",name,result);
}

function protocal_set_resystem(name,values) {
    return protocal_checkresult("set",name,protocal_callcgi_resystem("set:" + name + "&" + values));
}
/**
...
function protocal_callcgi_resystem(strParameter) {
